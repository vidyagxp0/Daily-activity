...
use App\Models\User;
use App\Models\GlobalCapaAuditTrail;
use App\Models\NotificationUser;
use App\Models\RoleGroup;
use App\Models\GlobalCapaGrid;
...
class GlobalCapaController extends Controller
{
    public function globalCapa()
    {
        $cft = [];
        $old_records = GlobalCapa::select('id', 'division_id', 'record')->get();
        ...
        if ($division) {
            $last_capa = GlobalCapa::where('division_id', $division->id)->latest()->first();
        ...
        return view("frontend.forms.global-capa", compact('due_date', 'record_number', 'old_records', 'cft'));
    }

    public function globalCapaStore(Request $request)
    {
        // return $request;
        ...
        if (!empty($capa->record)) {
            $history = new GlobalCapaAuditTrail();
    ...
    public function globalcapaUpdate(Request $request, $id)
    {
        if (!$request->short_description) {
            toastr()->error("Short description is required");
            return redirect()->back();
        }
        $lastDocument = GlobalCapa::find($id);
        $capa = GlobalCapa::find($id);
        ...
        if ($request->product_name) {
            $data1 = GlobalCapaGrid::where('capa_id', $id)->where('type', "Product_Details")->first();
            $data1->capa_id = $capa->id;
            $data1->type = "Product_Details";
        ...
        if ($request->material_name) {
            $data2 = GlobalCapaGrid::where('type', 'Material_Details')->where('capa_id', $id)->first();
            if (empty($data2)) {
                $data2 = new GlobalCapaGrid();
            }
        ...
        if ($request->equipment) {
            $data3 = GlobalCapaGrid::where('capa_id', $id)->where('type', "Instruments_Details")->first();
            $data3->capa_id = $capa->id;
            $data3->type = "Instruments_Details";
            if (!empty($request->equipment)) {
                $data3->equipment = serialize($request->equipment);
    ...
    public function globalCapaShow($id)
    {
        $cft = [];
        $revised_date = "";
        $data = GlobalCapa::find($id);
        //dd($data);
        $old_record = GlobalCapa::select('id', 'division_id', 'record')->get();
        ...
        $data1 = GlobalCapaGrid::where('capa_id', $id)->where('type', "Product_Details")->first();
        $data2 = GlobalCapaGrid::where('capa_id', $id)->where('type', "Material_Details")->first();
        $data3 = GlobalCapaGrid::where('capa_id', $id)->where('type', "Instruments_Details")->first();
        ...
        // dd($data);
        return view('frontend.Global-Capa.GlobalcapaView', compact('data', 'data1', 'data2', 'data3', 'old_record', 'revised_date', 'cft'));
    }


    public function global_capa_send_stage(Request $request, $id)
    {
    ...
    public function globalCapaCancel(Request $request, $id)
    {
        if ($request->username == Auth::user()->Username && Hash::check($request->password, Auth::user()->password)) {
            $capa = GlobalCapa::find($id);
            $lastDocument = GlobalCapa::find($id);
    ...
    public function global_qa_more_info(Request $request, $id)
    {
        if ($request->username == Auth::user()->Username && Hash::check($request->password, Auth::user()->password)) {
            $capa = GlobalCapa::find($id);
            $lastDocument = GlobalCapa::find($id);
    ...
    public function global_capa_reject(Request $request, $id)
    {
    ...
    public function GlobalCapaAuditTrial($id)
    {
        $audit = GlobalCapaAuditTrail::where('capa_id', $id)->orderByDESC('id')->paginate();
        $today = Carbon::now()->format('d-m-y');
        $document = GlobalCapa::where('id', $id)->first();
        $document->initiator = User::where('id', $document->initiator_id)->value('name');
        $data = GlobalCapa::find($id);

        // return $audit;

        return view('frontend.Global-Capa.audit-trial', compact('audit', 'document', 'today', 'data'));
    }

    public function auditDetails($id)
    {

        $detail = GlobalCapaAuditTrail::find($id);

        $detail_data = GlobalCapaAuditTrail::where('activity_type', $detail->activity_type)->where('capa_id', $detail->capa_id)->latest()->get();

        $doc = GlobalCapa::where('id', $detail->capa_id)->first();

        $doc->origiator_name = User::find($doc->initiator_id);
        return view('frontend.Global-Capa.audit-trial-inner', compact('detail', 'doc', 'detail_data'));
    }

    public function child_global_control(Request $request, $id)
    {
        $cft = [];
        $parent_id = $id;
        $parent_type = "Global CAPA";
        $record = ((RecordNumber::first()->value('counter')) + 1);
        $record = str_pad($record, 4, '0', STR_PAD_LEFT);
        $currentDate = Carbon::now();
        $formattedDate = $currentDate->addDays(30);
        $due_date = $formattedDate->format('d-M-Y');
        $parent_record = GlobalCapa::where('id', $id)->value('record');
        $parent_record = str_pad($parent_record, 4, '0', STR_PAD_LEFT);
        $parent_division_id = GlobalCapa::where('id', $id)->value('division_id');
        $parent_initiator_id = GlobalCapa::where('id', $id)->value('initiator_id');
        $parent_intiation_date = GlobalCapa::where('id', $id)->value('intiation_date');
        $parent_short_description = GlobalCapa::where('id', $id)->value('short_description');
        ...
        if ($request->child_type == "Action_Item") {
            $parentRecord = GlobalCapa::where('id', $id)->value('record');
            $parent_name = "Global CAPA";
            $data = GlobalCapa::find($id);
    ...
    public static function singleReport($id)
    {
        ...
        if (!empty($data)) {
            $data->Product_Details = GlobalCapaGrid::where('capa_id', $id)->where('type', "Product_Details")->first();
            $data->Instruments_Details = GlobalCapaGrid::where('capa_id', $id)->where('type', "Instruments_Details")->first();
            $data->Material_Details = GlobalCapaGrid::where('capa_id', $id)->where('type', "Material_Details")->first();
            ...
            $time = Carbon::now();
            $pdf = PDF::loadview('frontend.Global-Capa.singleReport', compact('data', 'capa_teamNamesString'))
                ->setOptions([
    ...
    {
        $data = GlobalCapa::find($id);
        return view('frontend.Global-Capa.capa_showpdf', compact('id', 'data'));
    }
...
// dd($data);
            ...
            $pdf = App::make('dompdf.wrapper');
            $time = Carbon::now();
            $pdf = PDF::loadview('frontend.Global-Capa.global_capa_family_report', compact('data', 'capa_teamNamesString', 'Extension', 'ActionItem', 'EffectivenessCheck'))
                ->setOptions([
                    'defaultFont' => 'sans-serif',
                    'isHtml5ParserEnabled' => true,
                    'isRemoteEnabled' => true,
            ...
            $canvas->page_script('$pdf->set_opacity(0.1,"Multiply");');
            $canvas->page_text($width / 4, $height / 2, $data->status, null, 25, [0, 0, 0], 2, 6, -20);
            return $pdf->stream('Global CAPA' . $id . '.pdf');
        }
    }

    public static function auditReport($id)
    {
        $doc = GlobalCapa::find($id);
        if (!empty($doc)) {
            $doc->originator = User::where('id', $doc->initiator_id)->value('name');
            $data = GlobalCapaAuditTrail::where('capa_id', $id)->get();
            $pdf = App::make('dompdf.wrapper');
            $time = Carbon::now();
            $data = $data->sortBy('created_at');
            $pdf = PDF::loadview('frontend.Global-Capa.auditReport', compact('data', 'doc'))
                ->setOptions([
                    'defaultFont' => 'sans-serif',
                    'isHtml5ParserEnabled' => true,
                    'isRemoteEnabled' => true,
