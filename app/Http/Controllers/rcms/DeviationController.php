    ...
    public static function familyReport($id)
    {
     ...
     public function exportCsv(Request $request){
        ...
        $deviation = $query->get();
            ...
            $callback = function () use ($deviation, $columns) {
                ...
                if ($deviation->isEmpty()) {
                    ...
                    foreach ($deviation as $index => $row) {
                        $data = [
                            $index + 1, // Sr. No.
                            $row->intiation_date ?? 'Not Applicable', 
                            $row->division ? $row->division->name . '/Deviation/' . date('Y') . '/' . str_pad($row->record, 4, '0', STR_PAD_LEFT) : 'Not Applicable',
                            $row->division ? $row->division->name : 'Not Applicable', 
                            Helpers::getFullDepartmentName($row->Initiator_Group) ?: ' Not Applicable ', 
                ...
                $callback = function () use ($deviation, $columns) {
                    ...
                    if ($deviation->isEmpty()) {
                        ...
                        foreach ($deviation as $index => $row) {
                            ...
                            echo "<td style='padding: 5px;'>" . htmlspecialchars($row->division ? $row->division->name . '/Deviation/' . date('Y') . '/' . str_pad($row->record, 4, '0', STR_PAD_LEFT) : 'Not Applicable') . "</td>";
                            echo "<td style='padding: 5px;'>" . htmlspecialchars( $row->division ? $row->division->name : 'Not Applicable') . "</td>";
                            echo "<td style='padding: 5px;'>" . htmlspecialchars(Helpers::getFullDepartmentName($row->Initiator_Group) ?: ' Not Applicable ') . "</td>";
