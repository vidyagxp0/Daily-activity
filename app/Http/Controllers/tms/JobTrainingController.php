    ...
    public function getSopDescription($id)
{
    ...
    public function store(Request $request)
    {
        ...
        $jobTraining->per_screen_running_time = $request->input('per_screen_running_time') ?? 0;
        $jobTraining->total_minimum_time = $request->input('total_minimum_time') ?? 0;
        ...
        //         }
        //     }
        //     $jobTraining->supporting_attachments1 = json_encode($files);
        // }
      ...
      $jobTraining_id = $jobTraining->id;
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            if ($request->hasFile($attachmentKey)) {
                ...
                if ($jobTraining->$attachmentKey) {
                    Storage::delete('public/' . $jobTraining->$attachmentKey);
                }
    
                $file = $request->file($attachmentKey);
                $filePath = $file->store('attachments', 'public');
                $jobTraining->$attachmentKey = $filePath;
            }
            ...
            if ($request->hasFile($attachmentKey)) {
                ...
                $jobTraining->$attachmentKey = $name;
            }
    ...
    public static function jobReport($id)
    {
        ...
        if (!empty($data)) {
            ...
            $canvas->page_script(function ($pageNumber, $pageCount, $canvas, $fontMetrics) {
                ...
                $yPosition = $canvas->get_height() * 0.06; // 9% from the top
    ...
    public function viewrendersop($id, $total_minimum_time, $per_screen_running_time, $job_training_id, $sop_spent_time){
        ...
        return view('frontend.TMS.on_the_job_training_detail', compact('id', 'totalMinimumTime', 'perScreenRunningTime', 'job_training_id', 'sop_spent_time'));
    }
...
public function showCertificate()
{
    ...
    return view('frontend.TMS.Job_Training.training_certificate');
}
