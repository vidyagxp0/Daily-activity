...
class RegulatoryController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // $internalAudit->regulatory_inspection_index = $request->regulatory_inspection_index;
...
// File Attachments
        ...
        $data2 = new RegulatoryGrid();
        $data2->audit_id = $internalAudit->id;
        $data2->type = "internal_audit";
        if (!empty($request->audit)) {
            $data2->area_of_audit = serialize($request->audit);
        }
        if (!empty($request->scheduled_start_date)) {
            $data2->start_date = serialize($request->scheduled_start_date);
        }
        if (!empty($request->scheduled_start_time)) {
            $data2->start_time = serialize($request->scheduled_start_time);
        }
        if (!empty($request->scheduled_end_date)) {
            $data2->end_date = serialize($request->scheduled_end_date);
        }
        if (!empty($request->scheduled_end_time)) {
            $data2->end_time = serialize($request->scheduled_end_time);
        }
        if (!empty($request->auditor1)) {
            $data2->auditor1 = serialize($request->auditor1);
        }
        if (!empty($request->auditee1)) {
            $data2->auditee1 = serialize($request->auditee1);
        }
        if (!empty($request->remark1)) {
            $data2->remark1 = serialize($request->remark1);
        }
...
if (!empty($internalAudit->record)) {
    ...
    $history->current = Helpers::getDivisionName($internalAudit->division_id) . '/RI/' . date('Y') .'/'.  str_pad($internalAudit->record, 4, '0', STR_PAD_LEFT) ;
    $history->comment = "NA";
...
if (!empty($internalAudit->priority_data)) {
    $history = new RegulatoryInspAuditTrail();
    $history->regulatory_id = $internalAudit->id;
    $history->activity_type = 'Priority Data';
    $history->previous = "Null";
    $history->current = $internalAudit->priority_data;
    $history->comment = "NA";
    $history->user_id = Auth::user()->id;
    $history->user_name = Auth::user()->name;
    $history->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
...
// new added supplieer audit trail code
    ...
    public function show($id)
    {
        ...
        // $data2 = RegulatoryGrid::where('audit_id', $id)->where('type', "Audit_Agenda")->firstOrCreate();
        $grid_data = RegulatoryGrid::where('audit_id', $id)->where('type', "internal_audit")->first();

        $grid_data1 = RegulatoryGrid::where('audit_id', $id)->where('type', "Observation_field_Auditee")->first();
        // dd($grid_data1);
        // foreach($sgrid as $s)
        // return $sgrid;
        return view('frontend.regulatory-inspection.regulatory_inspectionView', compact('data', 'old_record','sgrid','grid_data1','grid_data'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $internalAudit->initiator_group_code= $request->initiator_group_code;
        $internalAudit->short_description = $request->short_description;
        $internalAudit->priority_data = $request->priority_data;
        $internalAudit->audit_type = $request->audit_type;
        $internalAudit->if_other = $request->if_other;
        $internalAudit->Initiator_Group = $request->Initiator_Group;
        ...
        $data2 = RegulatoryGrid::where('audit_id',$internalAudit->id)->where('type','internal_audit')->first();
        ...
        if ($lastDocument->priority_data != $internalAudit->priority_data || !empty($request->priority_data)) {
...
// Due Date Extension
    ...
    public function AuditTrialSupplier($id)
    {
...
public static function auditReport($id)
{
    ...
    if (!empty($doc)) {
        ...
        $time = Carbon::now();
        $pdf = PDF::loadview('frontend.regulatory-inspection.auditReport_regulatory', compact('data', 'doc'))
            ->setOptions([
...
public static function regulatorySingleReport($id)
{
    ...
    //  dd($sgrid);
        ...
        $pdf = PDF::loadview('frontend.regulatory-inspection.regulatory_singleReport', compact('data','sgrid','grid_data1','grid_data'))
            ->setOptions([
                'defaultFont' => 'sans-serif',
                'isHtml5ParserEnabled' => true,
                'isRemoteEnabled' => true,
...
public function familyReport($id){
    ...
    if (!empty($data)) {
        ...
        $pdf = PDF::loadview('frontend.regulatory-inspection.regulatory_inspection_family_report', compact('data','sgrid','grid_data1','grid_data', 'Observation','griddata'))
            ->setOptions([
            'defaultFont' => 'sans-serif',
            'isHtml5ParserEnabled' => true,
            'isRemoteEnabled' => true,
        ...
        $pdf->save($filePath);
        return $pdf->stream('Regulatory' . $id . '.pdf');
    }
}
