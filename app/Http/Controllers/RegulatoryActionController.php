...
use App\Models\RegulatoryActionAuditTrail;
...
class RegulatoryActionController extends Controller
{

    public function showAction()
    {
        $old_record = RegulatoryAction::select('id', 'division_id', 'record')->get();
        $record = ((RecordNumber::first()->value('counter')) + 1);
        $record = str_pad($record, 4, '0', STR_PAD_LEFT);
        $currentDate = Carbon::now();
        $formattedDate = $currentDate->addDays(30);
        $due_date = $formattedDate->format('Y-m-d');
        return view('frontend.regulatory-action-task.regulatory-action-task', compact('due_date', 'record','old_record'));
    }
    ...
    public function Create(Request $request)
    {
        ...
        $openState = new RegulatoryAction();
        $openState->regulatoryaction_id = $request->ccId;
       ...
       // $openState->hod_preson = json_encode($request->hod_preson);
        ...
        if (!empty($openState->short_description)) {
            $history = new RegulatoryActionAuditTrail();
            $history->regulatoryaction_id = $openState->id;
       ...
       if (!empty($openState->comments)) {
        ...
        return redirect('rcms/regulatory_dashboard');
    }
    ...
    public function show($id)
    {
        ...
        $cc = CC::find($data->regulatoryaction_id);
        $data->record = str_pad($data->record, 4, '0', STR_PAD_LEFT);
        // $taskdetails = Taskdetails::where('regulatoryaction_id', $id)->first();
        // $checkeffec = CheckEffecVerifi::where('regulatoryaction_id', $id)->first();
        // $comments = RefInfoComments::where('regulatoryaction_id', $id)->first();
        // return $taskdetails;
        return view('frontend.regulatory-action-task.regulatory-actionView', compact('data', 'cc','old_record'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $lastopenState = RegulatoryAction::find($id);
        $openState = RegulatoryAction::find($id);
        ...
        if ($lastopenState->title != $openState->title || !empty($request->title_comment)) {
            $history = new RegulatoryActionAuditTrail;
            $history->regulatoryaction_id = $id;
            $history->activity_type = 'Title';
    ...
    public function stageChange(Request $request, $id)
    {
        ...
        if ($request->username == Auth::user()->email && Hash::check($request->password, Auth::user()->password)) {
            $changeControl = RegulatoryAction::find($id);
            $lastopenState = RegulatoryAction::find($id);
            $openState = RegulatoryAction::find($id);
            // $task = Taskdetails::where('regulatoryaction_id', $id)->first();
...
// {
//     if ($request->username == Auth::user()->email && Hash::check($request->password, Auth::user()->password)) {
//         $RegulatoryAction = RegulatoryAction::find($id);

//         $RegulatoryAction->status = "Closed-Cancelled";
//         $RegulatoryAction->cancelled_by = Auth::user()->name;
//         $RegulatoryAction->cancelled_on = Carbon::now()->format('d-M-Y');
...
//         $history->user_id = Auth::user()->id;
//         $history->user_name = Auth::user()->name;
//         $history->status = $RegulatoryAction->status;
//         $history->save();
...
public function RegulatoryActionAuditTrialShow($id)
{
    $audit = RegulatoryActionAuditTrail::where('regulatoryaction_id', $id)->orderByDESC('id')->paginate();
    $today = Carbon::now()->format('d-m-y');
    $document = RegulatoryAction::where('id', $id)->first();
    $document->initiator = User::where('id', $document->initiator_id)->value('name');

    return view('frontend.regulatory-action-task.audit-trial', compact('audit', 'document', 'today'));
}

public function RegulatoryActionAuditTrialDetails($id)
{
    $detail = RegulatoryActionAuditTrail::find($id);

    $detail_data = RegulatoryActionAuditTrail::where('activity_type', $detail->activity_type)->where('regulatoryaction_id', $detail->regulatoryaction_id)->latest()->get();

    $doc = RegulatoryAction::where('id', $detail->regulatoryaction_id)->first();
...
public static function singleReport($id)
{
    ...
    if (!empty($data)) {
        ...
        $time = Carbon::now();
        $pdf = PDF::loadview('frontend.regulatory-action-task.singleReport', compact('data'))
            ->setOptions([
        ...
        return $pdf->stream('RegulatoryAction' . $id . '.pdf');
    }
}
public static function auditReport($id)
{
    $doc = RegulatoryAction::find($id);
    if (!empty($doc)) {
        $doc->originator = User::where('id', $doc->initiator_id)->value('name');
        $data = RegulatoryActionAuditTrail::where('regulatoryaction_id', $id)->get();
        $pdf = App::make('dompdf.wrapper');
        $time = Carbon::now();
        $pdf = PDF::loadview('frontend\regulatory-action-task\regulatoryaction_audit_trail_pdf', compact('data', 'doc'))
            ->setOptions([
                'defaultFont' => 'sans-serif',
                'isHtml5ParserEnabled' => true,
                'isRemoteEnabled' => true,
        ...
        return $pdf->stream('RegulatoryAction-Audit' . $id . '.pdf');
    }
}
...
public function auditTrailPdf($id)
    {
        $doc = RegulatoryAction::find($id);
        $doc->originator = User::where('id', $doc->initiator_id)->value('name');
        $data = RegulatoryActionAuditTrail::where('regulatoryaction_id', $doc->id)->orderByDesc('id')->paginate();
       
        $pdf = App::make('dompdf.wrapper');
        $time = Carbon::now();
        $pdf = PDF::loadview('frontend.action-item.RegulatoryAction_audit_trail_pdf', compact('data', 'doc'))
            ->setOptions([
                'defaultFont' => 'sans-serif',
                'isHtml5ParserEnabled' => true,
                'isRemoteEnabled' => true,
