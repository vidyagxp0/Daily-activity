...
class InductionTrainingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $inductionTraining->total_minimum_time = $request->total_minimum_time ?? 0;
        $inductionTraining->per_screen_run_time = $request->per_screen_run_time ?? 0;
        ...
        // if (!empty($request->total_minimum_time)) {
        //     $validation2 = new InductionTrainingAudit();
        //     $validation2->induction_id = $inductionTraining->id;
        //     $validation2->activity_type = 'Total Minimum Time(In Minutes)';
        //     $validation2->previous = "Null";
        //     $validation2->current = \Carbon\Carbon::parse($request->total_minimum_time)->format('d-M-Y');
        //     $validation2->comment = "NA";
        //     $validation2->user_id = Auth::user()->id;
        //     $validation2->user_name = Auth::user()->name;
        //     $validation2->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
        ...
        // if (!empty($request->per_screen_run_time)) {
        //     $validation2 = new InductionTrainingAudit();
        //     $validation2->induction_id = $inductionTraining->id;
        //     $validation2->activity_type = 'Per Screen Runing Time(In Minutes)';
        //     $validation2->previous = "Null";
        //     $validation2->current = \Carbon\Carbon::parse($request->per_screen_run_time)->format('d-M-Y');
        //     $validation2->comment = "NA";
        //     $validation2->user_id = Auth::user()->id;
        //     $validation2->user_name = Auth::user()->name;
        //     $validation2->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($lastdocument->total_minimum_time != $inductionTraining->total_minimum_time) {
            $validation2 = new InductionTrainingAudit();
            $validation2->induction_id = $inductionTraining->id;
            $validation2->previous = $lastdocument->total_minimum_time;
            $validation2->current = $inductionTraining->total_minimum_time;
            $validation2->activity_type = 'Total Minimum Time(In Minutes)';
            $validation2->user_id = Auth::user()->id;
            $validation2->user_name = Auth::user()->name;
            $validation2->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
            ...
            $validation2->change_to =   "Not Applicable";
            $validation2->change_from = $lastdocument->status;
            if (is_null($lastdocument->total_minimum_time) || $lastdocument->total_minimum_time === '') {
                $validation2->action_name = 'New';
            } else {
                $validation2->action_name = 'Update';
            }
        ...
        if ($lastdocument->per_screen_run_time != $inductionTraining->per_screen_run_time) {
            $validation2 = new InductionTrainingAudit();
            $validation2->induction_id = $inductionTraining->id;
            $validation2->previous = $lastdocument->per_screen_run_time;
            $validation2->current = $inductionTraining->per_screen_run_time;
            $validation2->activity_type = 'Per Screen Runing Time(In Minutes)';
            $validation2->user_id = Auth::user()->id;
            $validation2->user_name = Auth::user()->name;
            $validation2->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
            ...
            $validation2->change_to =   "Not Applicable";
            $validation2->change_from = $lastdocument->status;
            if (is_null($lastdocument->per_screen_run_time) || $lastdocument->per_screen_run_time === '') {
                $validation2->action_name = 'New';
            } else {
                $validation2->action_name = 'Update';
            }
        ...
        try {
            ...
            if ($request->username == Auth::user()->email && Hash::check($request->password, Auth::user()->password)) {
                ...
                if ($jobTraining->stage == 5) {
                    ...
                    $jobTraining->qa_head_approval_complete_by = Auth::user()->name;
                    $jobTraining->qa_head_approval_complete_on = Carbon::now()->format('d-m-Y');
                    $jobTraining->qa_head_approval_complete_comment = $request->comment;
                ...
                if ($jobTraining->stage == 6) {
                    ...
                    $jobTraining->Send_To_OJT_by= Auth::user()->name;
                    $jobTraining->Send_To_OJT_on = Carbon::now()->format('d-m-Y');
                    $jobTraining->Send_To_OJT_comment = $request->comment;
                ...
                if ($jobTraining->stage == 7) {
                    ...
                    $jobTraining->creation_complete_by = Auth::user()->name;
                    $jobTraining->creation_complete_on = Carbon::now()->format('d-m-Y');
                    $jobTraining->creation_complete_comment = $request->comment;
    ...
    {
       $induction = Induction_training::find($id);
        ...
        if ($request->child_type == 'classroom_training') {
            return view('frontend.TMS.Classroom_Training.classroom_training', compact('induction','due_date','record','data','hods','delegate', 'mainvalue'));
        } else {
    ...
    // public function Induction_Child(Request $request, $id)
    // {
    //     $employees = Employee::all(); // Fetch all employees for selection
    //     $inductionTrainingDetails = Induction_training::where('employee_id', $id)->first(); // Fetch induction training details if available
    ...
    public static function inductionReport($id)
    {
        ...
        if (!empty($data)) {
            ...
            $canvas->page_script(function ($pageNumber, $pageCount, $canvas, $fontMetrics) {
                ...
                $fontSize = 10;
                $xPosition = 515; // Add 10px margin from the left
                $yPosition = $canvas->get_height() * 0.06; // 5.5% from the top in pixels
    ...
    public function viewrendersopinduction($id, $total_minimum_time, $per_screen_running_time, $job_training_id, $sop_spent_time){
        ...
        return view('frontend.TMS.induction_training_detail', compact('id', 'totalMinimumTime', 'perScreenRunningTime', 'job_training_id', 'sop_spent_time'));
    }
    ...
    public function saveReadingTime(Request $request)
    {
        ...
        $id = $request->input('id');
        $jobTraining = Induction_training::findOrFail($id);
