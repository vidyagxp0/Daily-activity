...
class OOSMicroController extends Controller
{
    ...
    //    record update
        ...
        if (!empty($request->division_id)) {
            ...
            $history->previous = "Null";
            $history->current = $request->division_id;
            $history->comment = "NA";
            $history->user_id = Auth::user()->id;
            $history->user_name = Auth::user()->name;
            $history->user_role = RoleGroup::where('id', Auth::user()->role)->value('name');
    ...
    foreach ($general_information as $key => $value) {
        ...
        if ($lastValue != $requestValue) {
            ...
            if (is_null($lastDocument->requestValue) || $lastDocument->requestValue === '') {
...
foreach ($Preliminary_Lab_Investigation as $key => $value){
    ...
    if($lastDocument->$key != $request->$key){
        ...
        $history->change_from = $lastDocument->status;
        $history->change_to =   $lastDocument->status;
        if (is_null($lastDocument->$key) || $lastDocument->$key === '') {
            $history->action_name = "New";
        } else {
            $history->action_name = "Update";
        }        
        ...
        if ($request->username == Auth::user()->email && Hash::check($request->password, Auth::user()->password)) {
            ...
            if ($changestage->stage == 1) {
                ...
                $changestage->comment_pending_initial_assessment = $request->comment;
                                ...
                                if (is_null($lastDocument->completed_by_pending_initial_assessment) || $lastDocument->completed_by_pending_initial_assessment === '') {
                                    $history->action_name = "New";
                                } else {
                                    $history->action_name = "Update";
                                }        
            ...
            if ($changestage->stage == 2) {
                ...
                $changestage->comment_under_phaseI_investigation = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_phaseI_investigation) || $lastDocument->completed_by_under_phaseI_investigation === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                            $history->save();
            ...
            if ($changestage->stage == 3) {
                ...
                $changestage->comment_under_phaseIB_investigation = $request->comment;
                            ...
                            if (is_null($lastDocument->completed_by_under_phaseIB_investigation) || $lastDocument->completed_by_under_phaseIB_investigation === '') {
                                $history->action_name = "New";
                            } else {
                                $history->action_name = "Update";
                            }                               $history->save();
            ...
            if ($changestage->stage == 5) {
                ...
                $changestage->comment_under_hypothesis = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_hypothesis) || $lastDocument->completed_by_under_hypothesis === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       $history->save();
            ...
            if ($changestage->stage == 6) {
                ...
                $changestage->comment_under_phaseII_investigation = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_phaseII_investigation) || $lastDocument->completed_by_under_phaseII_investigation === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
            ...
            if ($changestage->stage == 8) {
                ...
                $changestage->comment_under_manufacturing_investigation_phaseIIA = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_manufacturing_investigation_phaseIIA) || $lastDocument->completed_by_under_manufacturing_investigation_phaseIIA === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
            ...
            if ($changestage->stage == 9) {
                ...
                $changestage->comment_under_phaseIIB_additional_lab_investigation = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_phaseIIB_additional_lab_investigation) || $lastDocument->completed_by_under_phaseIIB_additional_lab_investigation === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
            ...
            if ($changestage->stage == 11) {
                ...
                $changestage->comment_approval_completed = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_approval_completed) || $lastDocument->completed_by_approval_completed === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
            ...
            if($changestage->stage == 13) {
                ...
                $changestage->comment_close_done = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_close_done) || $lastDocument->completed_by_close_done === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
        ...
        if ($request->username == Auth::user()->email && Hash::check($request->password, Auth::user()->password)) {
            ...
            if ($changestage->stage == 3) {
                ...
                $changestage->comment_under_phaseI_correction = $request->comment;
                            ...
                            if (is_null($lastDocument->completed_by_under_phaseI_correction) || $lastDocument->completed_by_under_phaseI_correction === '') {
                                $history->action_name = "New";
                            } else {
                                $history->action_name = "Update";
                            }                       
            ...
            if ($changestage->stage == 6) {
                ...
                $changestage->comment_under_repeat_analysis = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_repeat_analysis) || $lastDocument->completed_by_under_repeat_analysis === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
            ...
            if ($changestage->stage == 9) {
                ...
                $changestage->comment_under_phaseIIA_correction = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_phaseIIA_correction) || $lastDocument->completed_by_under_phaseIIA_correction === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
            ...
            if ($changestage->stage == 10) {
                ...
                $changestage->comment_under_batch_disposition = $request->comment;
                    ...
                    if (is_null($lastDocument->completed_by_under_batch_disposition) || $lastDocument->completed_by_under_batch_disposition === '') {
                        $history->action_name = "New";
                    } else {
                        $history->action_name = "Update";
                    }                       
