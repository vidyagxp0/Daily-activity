...
@section('rcms_container')
    ...
    <style>
        ...
        #division_id_samplestability{
            border: 1px solid #bfd0f2 !important;
            border-radius: 2px !important; 
        } 
        #date_fromsamplestability{
            border: 1px solid #bfd0f2 !important;
            border-radius: 2px !important;   
        } 
        #date_tosamplestability{
            border: 1px solid #bfd0f2 !important;
            border-radius: 2px !important;    
        }
        .btn-primary{
                    ...
                    <div style="padding: 5px;" class="scope-bar d-flex justify-content-end py-1">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('exportsamplestability.csv') }}">
                                    <i class="fas fa-file-csv"></i> CSV Export
                                </a>
                                <a class="dropdown-item" href="{{ route('exportsamplestability.excel') }}">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div style="    display: grid;    grid-template-columns: 1fr 1fr 1fr 1fr 1fr;" class="filter-bar row">
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <select class="custom-select" id="division_id_samplestability">
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <input type="date" class="custom-select" id="date_fromsamplestability">
                                                </div>
                        
                                                <!-- End Date Filter -->
                                                <div class=" mb-3">
                                                    <label for="date_to_deviation">End Date:</label>
                                                    <input type="date" class="custom-select" id="date_tosamplestability">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <table class="table table-bordered table-hover" style="background-color: #fff; border-collapse: collapse; text-align: left;">
                                    <thead style="background-color: #797979; color: #000;">
                                        <tr>
                                            ...
                                            <th class="sortable" style="background-color: #f5c27f" onclick="sortTable('intiation_date')">Initiation Date</th>
                                            <th style="background-color: #f5c27f">Record No.</th>
                                            <th class="sortable" style="background-color: #f5c27f" onclick="sortTable('division_id')">Division</th>
                                    ...
                                    <tbody id="tableData">
                                        @include('frontend.forms.Logs.filterData.samplestability_data')
                                    </tbody>
                                </table>
                                <!-- Loader -->
...
<script>
    ...
    async function sortTable(column) {
        ...
        try {
            const postUrl = "{{ route('api.samplestability.filter') }}";
...
<script>
    $(document).ready(function(){
        const filterData = {
            department_samplestability: [],
            division_id_samplestability: null,
            initiator_id_samplestability: [],
            date_fromsamplestability: null,
            date_tosamplestability: null,
            sort_column: '',
            sort_order: ''
        };
    
        $('#initiator_group').change(function() {
            filterData.department_samplestability = this.value;
            filterRecords();
        });
    
        $('#division_id_samplestability').change('change', function() {
            filterData.division_id_samplestability = this.value;
            filterRecords();
        });
    
        document.getElementById('initiator_id').addEventListener('change', function() {
            filterData.initiator_id_samplestability = this.value;
            filterRecords();
        });
    
        document.getElementById('date_fromsamplestability').addEventListener('change', function() {
            filterData.date_fromsamplestability = this.value;
            filterRecords();
        });
    
        document.getElementById('date_tosamplestability').addEventListener('change', function() {
            filterData.date_tosamplestability = this.value;
            filterRecords();
        });
    // ===============local=============
...
<script>
    ...
    const changerelateTo = document.getElementById('division_id_samplestability').value;
    const Initiator = document.getElementById('initiator_id').value;
    const dateFrom = document.getElementById('date_fromsamplestability').value;
    const dateTo = document.getElementById('date_tosamplestability').value;

    const url = `/api/sample-stabilityLog?department=${department}&changerelateTo=${changerelateTo}&Initiator=${Initiator}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
