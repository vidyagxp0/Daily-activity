...
@forelse ($analytics as $index => $doc)  

    <tr>
        <td>{{ $index + 1 }}</td>
        <td>{{ $doc->date_of_initiation ? \Carbon\Carbon::parse($doc->intiation_date)->format('d-M-Y') : 'Not Applicable' }}</td>
        <td>
            <a href="{{ route('analytics_qualification.show', $doc->id) }}" target="_blank" style="color: rgb(236,160,53); font-weight: bold; font-size: 16px;">
               {{ $doc->site_code ? $doc->site_code : ' Not Applicable ' }}/AQ/{{ date('Y') }}/{{ str_pad($doc->record_number, 4, '0', STR_PAD_LEFT) }}
            </a>
        </td>
