...
@forelse ($inventorymanagement as $index => $doc)  

    <tr>
        <td>{{ $index + 1 }}</td>
        <td>{{ $doc->intiation_date ? \Carbon\Carbon::parse($doc->intiation_date)->format('d-M-Y') : 'Not Applicable' }}</td>
        <td>
            <a href="{{ route('CC.show', $doc->id) }}" target="_blank" style="color: rgb(236,160,53); font-weight: bold; font-size: 16px;">
               {{ $doc->division ? $doc->division->name : ' Not Applicable ' }}/IM/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT) }}
            </a>
        </td>
