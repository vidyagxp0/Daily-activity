...
@forelse ($controlsample as $index => $doc)  

    <tr>
        <td>{{ $index + 1 }}</td>
        <td>{{ $doc->intiation_date ? \Carbon\Carbon::parse($doc->intiation_date)->format('d-M-Y') : 'Not Applicable' }}</td>
        <td>
            <a href="{{ route('controleSampleShow', $doc->id) }}" target="_blank" style="color: rgb(236,160,53); font-weight: bold; font-size: 16px;">
               {{ $doc->division ? $doc->division->name : ' Not Applicable ' }}/CS/{{ date('Y') }}/{{ str_pad($doc->record, 4, '0', STR_PAD_LEFT) }}
            </a>
        </td>
        <td>{{ $doc->division ? $doc->division->name : ' Not Applicable ' }}</td>
        <td>{{ $doc->market ?: ' Not Applicable ' }}</td>
...
<tr>
  ...
  <i class="fas fa-file-alt" style="color: white;"></i>
        ...
        <!-- Main Message -->
        <div style="font-size: 24px; font-weight: bold; color: rgb(236,160,53); text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.1);">
            No Data Available
        </div>
        <!-- Sub-Message -->
