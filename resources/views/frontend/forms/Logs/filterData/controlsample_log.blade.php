...
@section('rcms_container')
    ...
    <style>
        ...
        #division_id_controlsample{
            border: 1px solid #bfd0f2 !important;
            border-radius: 2px !important; 
        } 
        #date_fromcontrolsample{
            border: 1px solid #bfd0f2 !important;
            border-radius: 2px !important;   
        } 
        #date_tocontrolsample{
            border: 1px solid #bfd0f2 !important;
            border-radius: 2px !important;    
        }
        .btn-primary{
                    ...
                    <div style="padding: 5px;" class="scope-bar d-flex justify-content-end py-1">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('exportcsvcs') }}">
                                    <i class="fas fa-file-csv"></i> CSV Export
                                </a>
                                <a class="dropdown-item" href="{{ route('exportexcecs') }}">
                                    ...
                                    <div class="mt-1 mb-2 bg-white" style="height: auto; padding: 10px; margin: 5px;">
                                        <div class="d-flex align-items-center">
                                            <div style="    display: grid;    grid-template-columns: 1fr 1fr 1fr 1fr 1fr;" class="filter-bar row">
                                                <!-- Department Filter -->
                                                <div class=" mb-3">
                                                    <label for="initiator_group">Market</label>
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <select class="custom-select" id="division_id_controlsample">
                                                ...
                                                <div class=" mb-3">
                                                    ...
                                                    <input type="date" class="custom-select" id="date_fromcontrolsample">
                                                </div>
                        
                                                <!-- End Date Filter -->
                                                <div class=" mb-3">
                                                    <label for="date_to_deviation">End Date:</label>
                                                    <input type="date" class="custom-select" id="date_tocontrolsample">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <table class="table table-bordered table-hover" style="background-color: #fff; border-collapse: collapse; text-align: left;">
                                    <thead style="background-color: #797979; color: #000;">
                                        <tr>
                                            ...
                                            <th class="sortable" style="background-color: #f5c27f" onclick="sortTable('Initiator_Group')">Market</th>
                                    ...
                                    <tbody id="tableData">
                                        @include('frontend.forms.Logs.filterData.controlsample_data')
                                    </tbody>
                                </table>
                                <!-- Loader -->
...
<script>
    ...
    async function sortTable(column) {
        ...
        try {
            const postUrl = "{{ route('api.controlsample.filter') }}";
...
<script>
    $(document).ready(function(){
        const filterData = {
            department_controlsample: [],
            division_id_controlsample: null,
            initiator_id_controlsample: [],
            date_fromcontrolsample: null,
            date_tocontrolsample: null,
            sort_column: '',
            sort_order: ''
        };
    
        $('#initiator_group').change(function() {
            filterData.department_controlsample = this.value;
            filterRecords();
        });
    
        $('#division_id_controlsample').change('change', function() {
            filterData.division_id_controlsample = this.value;
            filterRecords();
        });
    
        document.getElementById('initiator_id').addEventListener('change', function() {
            filterData.initiator_id_controlsample = this.value;
            filterRecords();
        });
    
        document.getElementById('date_fromcontrolsample').addEventListener('change', function() {
            filterData.date_fromcontrolsample = this.value;
            filterRecords();
        });
    
        document.getElementById('date_tocontrolsample').addEventListener('change', function() {
            filterData.date_tocontrolsample = this.value;
            filterRecords();
        });
    // ===============local=============
...
<script>
    ...
    const changerelateTo = document.getElementById('division_id_controlsample').value;
    const Initiator = document.getElementById('initiator_id').value;
    const dateFrom = document.getElementById('date_fromcontrolsample').value;
    const dateTo = document.getElementById('date_tocontrolsample').value;
  
    const url = `/api/control-sampleLog?department=${department}&changerelateTo=${changerelateTo}&Initiator=${Initiator}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
