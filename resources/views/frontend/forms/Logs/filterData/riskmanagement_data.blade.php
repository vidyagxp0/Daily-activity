...
@forelse ($riskmlog as $logs)
    ...
    <tr>
        <td>{{$loop->index+1}}</td>
        <td>
            {{$logs->intiation_date ? \Carbon\Carbon::parse($logs->intiation_date)->format('d-M-Y') : 'Not Applicable'}}
        </td>
        <td> <a href="{{ route('showRiskManagement', $logs->id) }}" target="_blank" style="color: rgb(236,160,53); font-weight: bold; font-size: 16px;">
        {{ $logs->division ? $logs->division->name : 'Not Available' }}/RA/{{ date('Y') }}/{{ str_pad($logs->record, 4, '0', STR_PAD_LEFT)}}</a></td>
        <td>{{ $logs->division ? $logs->division->name : 'Not Available' }}</td>
        <td>{{ Helpers::getInitiatorGroupFullName($logs->Initiator_Group) ?: ' Not Applicable ' }}</td>
