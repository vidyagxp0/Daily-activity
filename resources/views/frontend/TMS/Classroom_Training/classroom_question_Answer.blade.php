...
<div class="quiz-container">
    ...
    @else
        ...
        <h1>Classroom Training Quiz</h1>

        <form action="{{ route('check_answer_classroom') }}" method="POST" id="quizForm"> 
            @csrf  
            ...
            <input type="hidden"  name="training_type" value="Classroom Training">
            <input type="hidden"  name="department" value="{{ $onthejobid->department }}">
            ...
            <div class="question {{ $index === 0 ? '' : 'hidden' }}" data-question-index="{{ $index }}">
                <p class="question-text"><strong>Question {{ $index + 1 }}:</strong> {{ $question->question }}
                    (<span class="question-type">{{ $question->type }}</span>)</p><!-- Display index here -->
