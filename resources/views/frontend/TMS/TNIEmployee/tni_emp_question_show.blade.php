...
<div class="quiz-container">
    ...
    @else
        ...
        <h1>TNI Employee Quiz</h1>
        <form action="{{ route('check_answer_tni_emp') }}" method="POST">
            @csrf
            {{-- {{ dd($inductiontrainingid->id, $inductiontrainingid->name_employee); }} --}}
            <input type="hidden"  name="training_id" value="{{ $tniempid->id }}">
            <input type="hidden" name="emp_id" value="{{ $tniempid->employee_code }}">
            <input type="hidden" name="employee_name" 
               value="{{ $tniempid->employee_name }}">
            <input type="hidden"  name="training_type" value="TNI Employee">
            <input type="hidden"  name="attempt_count" value="{{ $tniempid->attempt_count }}">
