...
<div class="quiz-container">
    ...
    @else
        ...
        <h1>TNI Matrix Quiz</h1>
        <form action="{{ route('check_answer_tni_matrix') }}" method="POST" id="quizForm">
            @csrf
            {{-- {{ dd($inductiontrainingid->id, $inductiontrainingid->name_employee); }} --}}
            <input type="hidden"  name="training_id" value="{{ $tnimid->id }}">
            <input type="hidden" name="emp_id" value="{{ Auth::guard('employee')->user()->full_employee_id }}">
            <input type="hidden" name="designation" value="{{ Auth::guard('employee')->user()->job_title }}">
            <input type="hidden" name="employee_name" 
               value="{{ Auth::guard('employee')->check() ? Auth::guard('employee')->user()->employee_name : 'Guest' }}">
            <input type="hidden"  name="training_type" value="TNI Matrix">
            <input type="hidden"  name="attempt_count" value="{{ $tnimid->attempt_count }}">
