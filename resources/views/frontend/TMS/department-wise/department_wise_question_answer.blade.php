...
<div class="quiz-container">
    @if ($departmentwiseid->attempt_count == -1)
    ...
    @else
        ...
        <h1>Job Role Quiz</h1>
        <form action="{{ route('check_answer_department_wise') }}" method="POST">
            @csrf
            {{-- {{ dd($inductiontrainingid->id, $inductiontrainingid->name_employee); }} --}}
            <input type="hidden"  name="training_id" value="{{ $departmentwiseid->id }}">
            <input type="hidden"  name="emp_id" value="{{ $departmentwiseid->employee_code }}">
            <input type="hidden"  name="employee_name" value="{{ Helpers::getEmpName($departmentwiseid->employee_name) }}">
            <input type="hidden"  name="training_type" value="Department Wise Job Role">
            <input type="hidden"  name="attempt_count" value="{{ $departmentwiseid->attempt_count }}">
